...
// Using the forEach method to print each name to the console
...
// CODE HERE
const citiesEndWithI = cities.filter(function(ele, i, arr) {
  return ele.endsWith("i");
});
console.log(citiesEndWithI);
...
fruits.forEach(function(fruits) {
  if (fruits.color === "red") {
    console.log(`The fruit with index ${fruits.name} is an apple.`);
  } else if (fruits.color === "orange") {
    console.log(`The fruit with index ${fruits.name} is an orange.`);
  } else {
    console.log(
      `The fruit with index ${fruits.name} is neither apple or orange.`
    );
  }
});
console.log(fruits);
...
// CODE HERE
const riceTag = foods.filter(function(foods) {
  return foods.tags.includes("rice");
});

// THE TOTAL
const total = riceTag.reduce(function(acc, curr) {
